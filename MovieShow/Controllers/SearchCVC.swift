...
//        collectionView.register(UINib(nibName: "BannerAddCellCV", bundle: nil), forCellWithReuseIdentifier: "BannerAddCellCV")
    ...
    func performSearch(keywords: String) {
        ...
        let urlStr = Constants.searchUrl + Constants.query + searchingKeywords.replacingOccurrences(of: " ", with: "%20")
        url = urlStr
        print("search url :\(url)")
        if var url = url{
            self.callTVSeasonsAPis(url : &url,isUserInteraction: false, isClear : true)
        }
