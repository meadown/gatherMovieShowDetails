...
class FavouriteVC: BaseVC {
    ...
    var clearAllBtn : UIBarButtonItem!
...
//        collectionView.register(UINib(nibName: "BannerAddCellCV", bundle: nil), forCellWithReuseIdentifier: "BannerAddCellCV")
        ...
        clearAllBtn = UIBarButtonItem(title: "Clear all", style: .plain, target: self, action: #selector(clearAllfvrts))
        clearAllBtn.tintColor = .white

        self.navigationItem.rightBarButtonItem = clearAllBtn
//        self.navigationController?.navigationItem.rightBarButtonItem = clearAllBtn

    }
    
    @objc func clearAllfvrts(){
        
        if CoreDataManager.shared.deleteAllFvrts() {
            fvrtTVs.removeAll()
            self.collectionView.reloadData()
        }else{
            okAlert(title: "Error", message: "Error while removing favourite")
        }
    }
...
//        }else{
            ...
            let item = fvrtTVs[indexPath.row]
