...
class TvMain : NSObject, NSCoding{

	var page : Int!
	var results : [TvMainResult]!
	var totalPages : Int!
	...
	init(fromDictionary dictionary: [String:Any]){
		...
		results = [TvMainResult]()
		if let resultsArray = dictionary["results"] as? [[String:Any]]{
			for dic in resultsArray{
				let value = TvMainResult(fromDictionary: dic)
                if !value.genreIds.contains(16){
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         results = aDecoder.decodeObject(forKey :"results") as? [TvMainResult]
