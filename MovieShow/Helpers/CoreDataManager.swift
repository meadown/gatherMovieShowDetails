...
class CoreDataManager: NSObject {
    ...
    func addFavourite(fvrtTV : FavouriteTVModel){
        ...
        if !isFavourite(id: fvrtTV.id){
            ...
            let entity = NSEntityDescription.entity(forEntityName: Constants.movieOrTV?.uppercased() ?? "TV", in: context)
    ...
    func getFavouriteList()-> [FavouriteTVModel]{
            let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
            let request = NSFetchRequest<NSFetchRequestResult>(entityName: Constants.movieOrTV?.uppercased() ?? "TV")
...
//                try context.save()
    ...
    func deleteAllFvrts(entityName: String = Constants.movieOrTV?.uppercased() ?? "TV")-> Bool {//to clean all tables of the database
            let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
            do{
                ...
                print("Cleaned the table named as \(entityName)")
                return true
            } catch { print("something went wrong while emptying table: \(entityName)") ; return false}
    ...
    func saveImage(data: Data , id : Int) {
        ...
        do {
            try data.write(to: directory.appendingPathComponent("\(Constants.movieOrTV?.uppercased() ?? "TV")\(id).png")!)
        } catch {
     ...
     func getSavedImage(id: Int, quality : JPEGQuality? = nil) -> UIImage? {
        if let dir = try? FileManager.default.url(for: .documentDirectory, in: .userDomainMask, appropriateFor: nil, create: false) {
            let image = UIImage(contentsOfFile: URL(fileURLWithPath: dir.absoluteString).appendingPathComponent("\(Constants.movieOrTV?.uppercased() ?? "TV")\(id)").path)
            return image
